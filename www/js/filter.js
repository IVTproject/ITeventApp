...
function filter_city(name_city) {
	...
	filter_object.push({name:name_city, ischecked:cheсk_box.checked});
}
...
function creat_filter(data) {
	...
	for(var i = 0; array_city[i]; i++) {
		html_text += " <input type='checkbox' class='checkbox' id='" + array_city[i] + "'" +
...
function get_text_format_filter() {
	...
	if(!is_first)
		return ("list_city=" + list_city + "&begin_date=" + begin_date + "&end_date=" + end_date);
	else
